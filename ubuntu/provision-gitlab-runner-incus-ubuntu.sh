...
gitlab_runner_version="${1:-16.11.1}"; shift || true
docker_version="${1:-26.1.3}"; shift || true
docker_compose_version="${1:-2.27.0}"; shift || true
...
# see incus image list images:${os_name,,}/ type=container architecture=${os_arch,,}
base_image_name="images:${os_name,,}/${os_codename}"

# delete the existing container.
if incus info "$image_name" >/dev/null 2>&1; then
    incus delete -f "$image_name"
fi

# build the container.
incus init $base_image_name $image_name </dev/null
incus config set $image_name boot.autostart=false
...
LXCEXEC
incus stop $image_name

# show the container configuration.
incus config show $image_name

# show the container btrfs properties.
